...
7. O aplicativo web Node.js usa o Cosmos DB como seu back-end. Tanto o Cosmos DB quanto o Serviço de Kubernetes do Azure enviam métricas para o Azure Monitor.
...
Para minimizar o volume de ataque, esse cenário não expõe a instância da VM Jenkins em HTTP. Para as tarefas de gerenciamento que exigem a interação com o Jenkins, você deve criar uma conexão remota segura usando um túnel SSH em seu computador local. Somente a autenticação de chave pública SSH é permitida para as instâncias de VM Grafana e Jenkins. Os logons com base em senha são desabilitados. Para obter mais informações, confira [Executar um servidor Jenkins no Azure](./jenkins.md).
...
- [Pequeno][small-pricing]: esse exemplo de preço refere-se a mil builds de contêiner por mês.
- [Médio][medium-pricing]: esse exemplo de preço refere-se a 100 mil builds de contêiner por mês.
- [Grande][large-pricing]: esse exemplo de preço refere-se a 1 milhão de builds de contêiner por mês.

## <a name="related-resources"></a>Recursos relacionados

Este cenário usou o Registro de Contêiner do Azure e o Serviço de Kubernetes do Azure para armazenar e executar um aplicativo baseado em contêiner. As Instâncias de Contêiner do Azure também podem ser usadas para executar aplicativos baseados em contêiner, sem precisar provisionar componentes de orquestração. Para obter mais informações, confira [Visão geral das Instâncias de Contêiner do Azure][docs-aci].
