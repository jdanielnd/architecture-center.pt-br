...
Este cenário mostra como implantar e operar um servidor de Jenkins empresarial escalonável, no Azure protegido com o logon único (SSO). A arquitetura também usa o Azure Monitor para monitorar o estado do servidor Jenkins. [**Implantar esta solução**](#deploy-the-solution).
...
- **Servidor Jenkins**. Uma máquina virtual é implantada para executar [Jenkins] [ azure-market] como um servidor de automação e serve como o mestre Jenkins. Para implantar essa VM, use o [modelo de solução para Jenkins no Azure][solution]. Há outras ofertas de Jenkins disponíveis no Azure Marketplace.
...
- **Sub-redes**. O servidor Jenkins está isolado em uma [sub-rede][subnet] para facilitar o gerenciamento e a segregação do tráfego de rede sem afetar o desempenho.
...
Para fornecer aos trabalhos Jenkins acesso aos recursos do Azure, um administrador do Microsoft Azure Active Directory cria entidades de serviço. Essas entidades concedem aos aplicativos &mdash; neste caso, os trabalhos do Jenkins &mdash; [acesso autenticado e autorizado][ad-sp] aos recursos do Azure.
