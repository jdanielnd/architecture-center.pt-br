...
> O [Microsoft Entity Framework](/ef) fornece recursos para repetir operações de banco de dados. Além disso, a maioria dos serviços do Azure e SDKs do cliente incluem um mecanismo de repetição. Para saber mais, consulte [Diretrizes de repetição para serviços específicos](/azure/architecture/best-practices/retry-service-specific).
