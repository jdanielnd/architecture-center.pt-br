...
O diagrama a seguir mostra a estrutura lógica de um exemplo de conta de armazenamento. A conta de armazenamento contém três tabelas: Informações de clientes, Informações de produtos e Informações de pedidos. 
...
* Se você executa regularmente consultas que pesquisam dados usando campos que não sejam as chaves de partição e de linha, considere a implementação do [padrão da tabela de índice] ou cogite usar um armazenamento de dados diferente que dê suporte ao índice, como o Cosmos DB.
...
[Execução do Redis em uma VM do Linux CentOS no Azure]: https://blogs.msdn.microsoft.com/tconte/2012/06/08/running-redis-on-a-centos-linux-vm-in-windows-azure/
