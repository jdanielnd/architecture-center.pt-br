...
- Nível 0: Defina um URI, e todas as operações serão solicitações POST para esse URI.
...
Isso instrui o servidor a atualizar `price`, excluir `color` e adicionar `size` &mdash; `name` e `category` não são modificados. Para obter os detalhes exatos do patch de mesclagem JSON, consulte [RFC 7396](https://tools.ietf.org/html/rfc7396). O tipo de mídia para patch de mesclagem JSON é `application/merge-patch+json`.
