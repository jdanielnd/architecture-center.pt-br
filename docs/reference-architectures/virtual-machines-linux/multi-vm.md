...
Essa arquitetura de referência mostra um conjunto de práticas comprovadas para executar várias VMs (máquinas de virtuais) do Linux em um conjunto de dimensionamento atrás de um balanceador de carga para melhorar a disponibilidade e a escalabilidade. Essa arquitetura pode ser usada para qualquer carga de trabalho sem estado, como um servidor Web e é um bloco de construção para a implantação de aplicativos de n camadas. [**Implantar esta solução**.](#deploy-the-solution)
...
Essa arquitetura baseia-se naquela mostrada em [Executar uma VM do Linux no Azure][single vm]. As recomendações descritas lá também se aplicam a essa arquitetura.

Nessa arquitetura, uma carga de trabalho é distribuída entre várias instâncias de VM. Há um único endereço IP público e o tráfego da Internet é distribuído para as VMs usando um balanceador de carga. Essa arquitetura pode ser usada para um aplicativo de camada única, como um aplicativo Web sem estado.

A arquitetura tem os seguintes componentes:

* **Grupo de recursos.** [*Grupos de recursos*][resource-manager-overview] são usados para agrupar os recursos para que eles possam ser gerenciados pelo tempo de vida, o proprietário e outros critérios.
* **Rede Virtual e sub-rede.** Cada VM no Azure é implantada em uma Rede Virtual, que é dividida em sub-redes.
* **Azure Load Balancer**. O [balanceador de carga] distribui as solicitações de entrada da Internet para as instâncias de VM. 
...
* **Conjunto de dimensionamento de VM**. Um [conjunto de dimensionamento de VM][vm-scaleset] é um conjunto de VMs idênticas usado para hospedar uma carga de trabalho. Conjuntos de dimensionamento permitem que o número de VMs seja reduzido horizontalmente manualmente ou com base em regras predefinidas.
...
Por padrão, os conjuntos de dimensionamento usam “excesso de provisionamento”, o que significa que o conjunto de dimensionamento inicialmente provisiona mais VMs do que você pediu, excluindo posteriormente as VMs adicionais. Isso melhora a taxa de sucesso geral ao provisionar as VMs. Se você não estiver usando [discos gerenciado](/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-managed-disks), recomendamos não usar mais do que 20 VMs por conta de armazenamento com excesso de provisionamento habilitado ou no máximo 40 VMs com o excesso de provisionamento desabilitado.
...
Coloque as VMs dentro da mesma sub-rede. Não exponha as VMs diretamente à Internet, concedendo, em vez disso, um endereço IP privado a cada VM. Os clientes se conectam usando um endereço IP público do balanceador de carga.

Se você precisar fazer logon em VMs por trás do balanceador de carga, considere adicionar uma única VM como um host bastião/jumpbox com um endereço IP público na qual você pode fazer logon. Em seguida, faça logon nas VMs por trás do balanceador de carga por meio do jumpbox. Outra opção é configurar as regras NAT de entrada no balanceador de carga para a mesma finalidade. No entanto, ter um jumpbox é a melhor solução quando você hospeda cargas de trabalho de n camadas ou várias cargas de trabalho.
...
Crie contas de Armazenamento do Azure separadas para cada VM para conter os VHDs (discos rígidos virtuais), de modo a evitar atingir os limites de [IOPS (operações de entrada/saída por segundo)][vm-disk-limits] para contas de armazenamento.

Também é recomendável usar os [discos gerenciado](/azure/storage/storage-managed-disks-overview) com o [armazenamento premium][premium]. Discos gerenciados não exigem uma conta de armazenamento. Você simplesmente especifica o tamanho e tipo de disco e ele é implantado de modo altamente disponível.
...
Para obter mais informações, consulte [Gerenciar a disponibilidade de máquinas virtuais][availability set]. O vídeo a seguir também apresenta uma boa visão geral dos conjuntos de disponibilidade: [Como configurar um conjunto de disponibilidade para dimensionar VMs][availability set ch9].
...
* A investigação é enviada de um endereço IP [conhecido][health-probe-ip], 168.63.129.16. Verifique se o tráfego de ou para esse IP não é bloqueado por quaisquer políticas de firewall ou regras de NSG (Grupo de Segurança de Rede).
...
Com várias VMs, é importante automatizar os processos para que eles sejam confiáveis e reproduzíveis. Você pode usar a [Automação do Azure][azure-automation] para automatizar a implantação, aplicação de patches do sistema operacional e outras tarefas.

## <a name="security-considerations"></a>Considerações de segurança

Redes virtuais são um limite de isolamento de tráfego no Azure. As VMs de uma VNet não podem se comunicar diretamente com as VMs de outra VNet. As VMs na mesma VNet podem se comunicar, a menos que você crie NSGs ([Grupos de Segurança de Rede][nsg]) para restringir o tráfego. Para obter mais informações, consulte [Serviços em nuvem da Microsoft e segurança de rede][network-security].
...
Uma implantação para essa arquitetura está disponível no [GitHub][github-folder]. Ela inclui uma VNet, NSG e três VMs em um conjunto de dimensionamento por trás de um balanceador de carga, descrito abaixo:

  * Uma rede virtual com uma única sub-rede denominada **Web** usada para hospedar as VMs.
  * Um conjunto de dimensionamento de VM que contém as VMs que executam a versão mais recente do Ubuntu 16.04.3 LTS. O dimensionamento automático está habilitado.
  * Um balanceador de carga localizado na frente do conjunto de dimensionamento da VM.
  * Um NSG com duas regras de entrada para permitir tráfego HTTP para o conjunto de dimensionamento da VM.
...
1. Navegue até a pasta `virtual-machines\multi-vm\parameters\linux` do repositório que você baixou na etapa de pré-requisitos acima.

2. Abra o arquivo `multi-vm-v2.json` e insira um nome de usuário e a chave SSH entre aspas, conforme mostrado abaixo e salve o arquivo.
