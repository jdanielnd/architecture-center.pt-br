...
Essa arquitetura baseia-se naquela mostrada em [Executar uma VM do Windows no Azure][single vm]. As recomendações descritas lá também se aplicam a essa arquitetura.
...
Com várias VMs, é importante automatizar os processos para que eles sejam confiáveis e reproduzíveis. Você pode usar a [Automação do Azure][azure-automation] para automatizar a implantação, aplicação de patches do sistema operacional e outras tarefas. A [Automação do Azure][azure-automation] é um serviço de automação baseado no Windows PowerShell que pode ser usado para isso. Scripts de automação de exemplo estão disponíveis na [Galeria de Runbook] no TechNet.
