...
description: Arquitetura recomendada para um aplicativo Web básico executado no Azure.
author: MikeWasson
ms.date: 12/12/2017
ms.custom: seodec18
...
ms.locfileid: "54114293"
---
# <a name="run-a-basic-web-application-in-azure"></a>Executar aplicativo Web básico no Azure

Essa arquitetura de referência mostra práticas comprovadas para um aplicativo Web que usa o [Serviço de Aplicativo do Azure][app-service] e o [Banco de Dados SQL do Azure][sql-db]. [**Implantar esta solução**](#deploy-the-solution).

![Arquitetura de referência para um aplicativo Web básico no Azure](./images/basic-web-app.png)
...
- *Expandir*, que significa adicionar instâncias para lidar com o aumento da carga. Cada tipo de preço tem um número máximo de instâncias.
  ...
  Para habilitar o dimensionamento automático, crie um *perfil* de dimensionamento automático que defina o número mínimo e máximo de instâncias. Os perfis podem ser agendados. Por exemplo, você pode criar perfis separados para dias da semana e finais de semana. Opcionalmente, um perfil pode conter regras de quando adicionar ou remover instâncias. (Exemplo: Adicione duas instâncias se o uso da CPU ficar acima de 70% durante 5 minutos.)
...
Caso seja necessário elevar uma camada de serviço ou um nível de desempenho do Banco de Dados SQL, você poderá aumentar os bancos de dados individuais, sem tempo de inatividade do aplicativo. Para saber mais, confira [Opções e desempenho do Banco de Dados SQL: Entenda o que está disponível em cada camada de serviço][sql-db-scale].
...
Execute testes de carga usando uma ferramenta como o [Azure DevOps][azure-devops] ou o [Visual Studio Team Foundation Server][tfs]. Para obter uma visão geral da análise de desempenho em aplicativos de nuvem, consulte [Performance Analysis Primer][perf-analysis] (Primer de análise de desempenho).
